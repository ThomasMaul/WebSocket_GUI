function rkmd_rangeSlider(selector) {
    ...
    self.on("mousedown touchstart", ".slider-handle", function (e) {
        ...
        var handlers = { mousemove: moveFu, touchmove: moveFu, mouseup: upFu, touchend: upFu };
    ...
    self.on("mousedown touchstart", ".slider", function (e) {
        ...
        var handlers = { mouseup: upFu, touchend: upFu };
        $(document).on(handlers);
    });
}
function sliderDiscrete_tmplt() {
    var tmplt = '<div class="slider">' + '<div class="slider-fill"></div>' + '<div class="slider-handle"><div class="slider-label"><span>0</span></div></div>' + "</div>";
    return tmplt;
}
function slider_move(parents, newW, sliderW, send) {
    ...
    range.next().html(newW);
    slider_fill.css("width", slider_new_val + "%");
    slider_handle.css({ left: slider_new_val + "%", transition: "none", "-webkit-transition": "none", "-moz-transition": "none" });
