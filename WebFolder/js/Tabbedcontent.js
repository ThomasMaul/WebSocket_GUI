(function ($, document, window, undefined) {
    ...
    var Tabbedcontent = function (tabcontent, options) {
        ...
        function filterTab(tab) {
            return $(this)
                .attr("href")
                .match(new RegExp(tab + "$"));
        }
        function getTab(tab) {
            if (tab instanceof $) {
                return { tab: tab, link: options.links.eq(tab.index()) };
            }
            if (isInt(tab)) {
                return { tab: children.eq(tab), link: options.links.eq(tab) };
            }
        ...
        function api() {
            return { switch: apiSwitch, switchTab: apiSwitch, getCurrent: getCurrent, getTab: getTab, next: next, prev: prev, isFirst: isFirst, isLast: isLast };
        }
        init();
        return api();
    };
